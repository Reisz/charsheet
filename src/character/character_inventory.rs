...
use crate::model::{InventoryId, InventoryInfo, ItemId};
...
impl CharacterInventory {
    ...
    fn limit_fill(&self, info: &InventoryInfo, amount: u16, capacity: Option<u32>) -> u16 {
        if let Some(capacity) = capacity {
            let capacity = u16::try_from(capacity - self.fill).unwrap_or(std::u16::MAX);
            min(amount, capacity / info.size)
        } else {
            amount
        }
    }
    ...
    pub(crate) fn put(
        ...
        let to_put = self.limit_fill(info, amount, capacity);
        let remainder = amount - to_put;

        let stack_size: u16 = info.stack_size.into();
        ...
        self.fill += u32::from(amount - remainder) * u32::from(info.size);
        remainder
    }
}
